    ...
    <div className="flex flex-col h-full bg-bg">
      ...
      <div className="p-4 border-b border-[var(--border)] shrink-0">
        ...
        <div className="relative">
          <svg
            className="w-5 h-5 absolute left-3 top-2.5 text-ink"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
          ...
          <input
            ...
            className="w-full bg-surface-strong border border-[var(--border)] rounded-xl py-2 pl-10 pr-4 text-sm focus:outline-none focus:border-purple transition-colors"
          />
        </div>
      </div>

      {/* Tabs */}
      <div className="flex overflow-x-auto p-2 gap-2 border-b border-[var(--border)] shrink-0 scrollbar-hide">
        {tabs.map((tab) => (
          <button
            key={tab}
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="p-4 border-b border-[var(--border)] flex gap-3 animate-pulse">
              <div className="w-12 h-12 bg-[var(--surface-strong)] rounded-full shrink-0" />
              <div className="flex-1">
                <div className="h-4 bg-[var(--surface-strong)] rounded w-1/3 mb-2" />
                <div className="h-3 bg-[var(--surface-strong)] rounded w-2/3" />
              </div>
            </div>
          ))
        ) : filteredConversations.length === 0 ? (
          <div className="p-8 text-center text-ink flex flex-col items-center">
            <svg className="w-12 h-12 mb-3 text-ink" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          filteredConversations.map((conv) => (
            <button
              ...
              className={`w-full text-left p-4 border-b border-[var(--border)] flex gap-3 transition-colors hover:bg-[var(--surface-strong)] relative ${
                activeId === conv._id ? "bg-surface-strong" : ""
              }`}
            >
              <div className="relative shrink-0">
                {conv.otherUser?.image ? (
                  ...
                  <div className="w-12 h-12 bg-surface-strong rounded-full flex items-center justify-center text-lg font-medium">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className={`text-sm truncate ${conv.unreadCount > 0 ? "text-ink font-medium" : "text-ink-soft"}`}>
                  {conv.lastMessage || "No messages yet"}
                </p>
                
                <div className="flex items-center gap-2 mt-2">
                  {conv.relatedEntityType && (
                    <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded bg-[var(--surface-strong)] text-ink font-medium">
