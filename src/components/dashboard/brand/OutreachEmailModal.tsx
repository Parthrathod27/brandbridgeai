      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-50 w-full max-w-2xl translate-x-[-50%] translate-y-[-50%] rounded-2xl border border-[var(--border)] bg-[#0a0a0a] p-6 shadow-2xl data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%]">
          <div className="flex items-center justify-between border-b border-[var(--border)] pb-4">
            ...
            <Dialog.Close className="rounded-full p-2 text-ink-faint hover:bg-[var(--surface-strong)] hover:text-ink">
              <X size={20} />
            </Dialog.Close>
          </div>
          ...
          <div className="py-6">
            {loading ? (
              <div className="flex flex-col items-center justify-center space-y-4 py-12">
                ...
                <p className="text-sm text-ink-soft">Crafting personalized outreach for {targetBrandName}...</p>
              </div>
            ) : (
          ...
          <div className="flex justify-end gap-3 border-t border-[var(--border)] pt-4">
            <button
              onClick={onClose}
              className="rounded-xl px-4 py-2 text-sm font-medium text-ink-soft transition hover:bg-[var(--surface-strong)]"
            >
              Close
            </button>
            <button
