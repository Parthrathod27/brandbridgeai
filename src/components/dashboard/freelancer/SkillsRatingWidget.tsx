    ...
    <div className="flex items-center gap-0.5">
      ...
      {half && (
        <div className="relative">
          <Star size={14} className="text-ink-faint" />
      ...
      {Array.from({ length: empty }).map((_, i) => (
        <Star key={`e${i}`} size={14} className="text-ink-faint" />
      ))}
    </div>
  );
}
  ...
  return (
    <div className="bb-glass bb-card-interactive rounded-2xl p-5 transition-all duration-200">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4">
        ...
        <div className="h-4 w-px bg-[var(--surface-strong)]" />
        <div className="flex items-center gap-1.5 text-xs text-ink-soft">
          <Award size={12} className="text-purple" />
        ...
        {meta.hourlyRate != null && meta.hourlyRate > 0 && (
          <>
            <div className="h-4 w-px bg-[var(--surface-strong)]" />
            <div className="flex items-center gap-1.5 text-xs text-ink-soft">
              <DollarSign size={12} className="text-purple" />
              ${meta.hourlyRate}/hr
            </div>
          </>
        )}
      ...
      {meta.categories.length > 0 && (
        <div className="mt-4">
          <p className="mb-2 flex items-center gap-1.5 text-[10px] font-medium uppercase tracking-wider text-ink-faint">
            <Tag size={10} />
            Categories
          </p>
          <div className="flex flex-wrap gap-1.5">
            {meta.categories.map((cat) => (
              <span
                key={cat}
                className="rounded-full bg-purple-500/15 px-2.5 py-1 text-[11px] font-medium text-purple"
              >
                {cat}
              </span>
            ))}
        ...
        <div className="mt-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {meta.skills.map((skill) => (
              <span
                key={skill}
                className="rounded-full border border-[var(--border)] bg-[var(--surface-strong)] px-2.5 py-1 text-[11px] text-ink-soft"
              >
                {skill}
              </span>
            ))}
