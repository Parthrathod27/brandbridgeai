        ...
        <div className="flex items-center justify-between p-6 border-b border-white/10 shrink-0">
          <div>
            <h2 className="bb-display text-2xl font-semibold flex items-center gap-3">
              {campaign.title}
              <span className="text-xs bg-purple-500/20 text-purple-300 px-2 py-1 rounded-md uppercase font-medium">
                {campaign.status}
              </span>
            </h2>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="bg-white/5 p-4 rounded-xl">
                <span className="text-[10px] uppercase tracking-wider text-white/40">Type</span>
              ...
              <div className="bg-white/5 p-4 rounded-xl">
                <span className="text-[10px] uppercase tracking-wider text-white/40">Start Date</span>
                <p className="text-sm mt-1">{formatDate(campaign.startDate)}</p>
              </div>
              <div className="bg-white/5 p-4 rounded-xl">
                <span className="text-[10px] uppercase tracking-wider text-white/40">End Date</span>
            ...
            <div className="border border-purple-500/20 bg-gradient-to-br from-purple-500/5 to-transparent rounded-2xl p-5">
              <h3 className="font-medium flex items-center gap-2 mb-4 text-purple-200">
              ...
              <div className="flex gap-3 mb-6">
                <button 
                  ...
                  className="bg-purple-500/20 text-purple-300 hover:bg-purple-500/30 px-4 py-2 rounded-xl text-sm flex items-center gap-2 transition-colors disabled:opacity-50"
                >
              ...
              {brief && (
                <div className="mb-6">
                  <h4 className="text-xs uppercase tracking-wider text-white/40 mb-2">Campaign Brief</h4>
                  <div className="bg-[#1a1a24] p-4 rounded-xl border border-white/5 text-sm text-white/80 whitespace-pre-wrap">
                    {brief}
                  </div>
                </div>
              )}
              ...
              {contentIdeas.length > 0 && (
                <div>
                  <h4 className="text-xs uppercase tracking-wider text-white/40 mb-2">Content Ideas</h4>
                  <div className="flex flex-wrap gap-2">
                    {contentIdeas.map((idea, idx) => (
                      <div key={idx} className="bg-[#1a1a24] border border-white/5 text-sm text-white/80 p-3 rounded-xl w-full">
                        {idea}
                      </div>
                    ))}
                  </div>
            ...
            <div>
              ...
              <div className="bg-white/5 border border-dashed border-white/20 rounded-xl p-8 text-center">
                <p className="text-sm text-white/40 mb-3">Upload visual assets, copy drafts, and references</p>
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-purple-500/10 p-3 rounded-xl flex justify-between items-center mt-4">
                  <span className="text-xs text-purple-300">Budget Spent</span>
            ...
            <div>
              ...
              <div className="space-y-4">
                {loadingLogs ? (
                  ...
                  logs.map((log) => (
                    <div key={log._id} className="relative pl-4 border-l border-white/10">
                      <div className="absolute left-0 top-1.5 w-1.5 h-1.5 bg-purple-500 rounded-full -translate-x-1/2"></div>
                      <p className="text-[11px] font-medium text-purple-300 mb-0.5">{log.action}</p>
                      <p className="text-xs text-white/60">{log.details}</p>
                      <p className="text-[10px] text-white/30 mt-1">{new Date(log.createdAt).toLocaleDateString()}</p>
                    </div>
                  ))
                )}
              </div>
