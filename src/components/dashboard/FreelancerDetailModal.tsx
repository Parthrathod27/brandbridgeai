            ...
            <div>
              ...
              <div className="mt-1 flex flex-wrap items-center gap-3 text-sm">
                ...
                {fProfile?.hourlyRate != null && (
                  <span className="text-ink-soft before:content-['•'] before:mr-3 before:text-ink-faint">${fProfile.hourlyRate}/hr</span>
                )}
                {profile?.location && (
                  <span className="text-ink-soft flex items-center gap-1 before:content-['•'] before:mr-3 before:text-ink-faint">
        ...
        <div className="p-6 overflow-y-auto flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <div>
              ...
              <p className="text-sm text-ink-soft whitespace-pre-wrap">{profile?.bio || "No bio provided."}</p>
            </div>

            <div>
              <h3 className="font-medium text-ink mb-3">Portfolio</h3>
              {freelancer.portfolio && freelancer.portfolio.length > 0 ? (
                ...
                <p className="text-sm text-ink-faint">No portfolio items available.</p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-6 border-t lg:border-t-0 lg:border-l border-[var(--border)] lg:pl-6 pt-6 lg:pt-0">
            <div>
              <h3 className="text-xs uppercase tracking-wider text-ink-faint mb-3">Skills & Categories</h3>
              <div className="flex flex-wrap gap-2">
                {[...(fProfile?.categories || []), ...(fProfile?.skills || [])].map((tag) => (
                  <span key={tag} className="rounded-full bg-[var(--surface-strong)] px-3 py-1 text-xs text-ink-soft">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            <div>
              <h3 className="text-xs uppercase tracking-wider text-ink-faint mb-3">Stats & Info</h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-ink-faint flex items-center gap-2"><CheckCircle size={14} className="text-green-400" /> Completed Projects</span>
                  <span className="font-medium text-ink">{fProfile?.completedProjects || 0}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-ink-faint flex items-center gap-2"><Calendar size={14} /> Availability</span>
                  <span className="font-medium text-ink capitalize">{fProfile?.availability || "N/A"}</span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-[var(--border)] space-y-3">
              ...
              <button onClick={() => { onClose(); onMessage(); }} className="w-full border border-[var(--border)] bg-[var(--surface-strong)] hover:bg-[var(--surface-strong)] transition-colors py-3 rounded-xl text-sm font-medium text-ink">
                Send Message
              </button>
            </div>
          </div>
