          ...
          {proposals.map((p) => (
            <div key={p._id} className="bb-glass rounded-2xl p-5">
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full bg-[var(--surface-strong)] px-2 py-0.5 text-[10px] capitalize text-purple">{p.status}</span>
              </div>
              {p.message && <p className="mt-2 text-sm text-ink-faint">{p.message}</p>}
              {p.rate != null && <p className="mt-2 text-xs text-ink-faint">Rate: ${p.rate}</p>}
            </div>
          ))}
        </div>
      )}
